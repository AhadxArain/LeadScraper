...
def _make_status(page, valid, rejected, existing, msg, done=False, **extra) -> dict:
    """Construct a uniform status dict for the generator consumer."""
    return {
        "page":           page,
        "valid_count":    valid,
...
def run_scraper(
    ...
    Yields status dicts on every meaningful action so the consumer
    (Streamlit UI or API) can display live progress without blocking.
    """
    ...
    # Collect in memory; batch-write to CSV at the very end
    valid_batch    = []
    rejected_batch = []
    existing_batch = []
    ...
    while valid_count < required_leads and page <= MAX_PAGES:
        ...
        # ── Process each result (with Layer-1 prefetch for the whole page) ───
        prefetch_pool = ThreadPoolExecutor(max_workers=2)
        layer1_futures = {}

        # Kick off Layer 1 crawls ahead of time for all places on this page
        for p in places:
            site = (p.get("website") or "").strip()
            if site:
                layer1_futures[id(p)] = prefetch_pool.submit(
                    extract_email_from_website, site
                )
        ...
        for place in places:
            ...
            # ── Step 2: Email extraction (Layer 1 prefetched, 2-4 parallel) ──
            email = None
            email_source = ""

            # Layer 1 — result may already be ready from prefetch pool
            if lead["website"]:
                yield _make_status(
                    page, valid_count, rejected_count, existing_count,
                    f"🔍 Scanning {biz} website…"
                )
                fut = layer1_futures.get(id(place))
                email = fut.result() if fut else extract_email_from_website(lead["website"])
                if email:
            ...
            # Confidence score (0-100) for quality ranking
